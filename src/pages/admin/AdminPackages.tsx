...
const AdminPackages = () => {
  ...
  const fetchPackages = async () => {
    ...
    setPackages(
      ((data as any) || []).map((p: any) => ({
        ...
        allow_custom_amount: p.allow_custom_amount ?? true,
      }))
    );
    setLoading(false);
  };
  ...
  const save = async (i: number) => {
    const p = packages[i];
    if (!p.slug || !p.name || p.price < 0) {
      toast.error("Slug, name and non-negative price required");
      return;
    }
    const payload: any = {
      slug: p.slug,
      ...
      allow_custom_amount: p.allow_custom_amount,
      installments: p.installments,
    ...
    if (p.id) {
      ...
      logAdminAction({
        ...
        description: `Updated package ${p.name}`,
        targetType: "ticket_package",
        targetId: p.id,
        metadata: payload,
      });
    } else {
      ...
      logAdminAction({
        ...
        description: `Created package ${p.name}`,
        targetType: "ticket_package",
        targetId: data?.id,
        metadata: payload,
      });
  ...
  const remove = async (i: number) => {
    ...
    logAdminAction({
      ...
      description: `Deleted package ${p.name}`,
      targetType: "ticket_package",
      targetId: p.id,
    });
  ...
  const addNew = () =>
    setPackages((prev) => [...prev, { ...blank(), display_order: prev.length + 1 }]);
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Edit prices, perks, capacity and installment plans. Installments are tied to the package
            price — when you change the price, update the installment amounts here too.
          </p>
        </div>
        <button
          onClick={addNew}
      ...
      <div className="space-y-4">
        {packages.map((p, i) => {
          const installmentTotal =
            p.installment_mode === "amount"
              ? p.installments.reduce((s, n) => s + Number(n || 0), 0)
              : Math.round(
                  p.installments.reduce((s, n) => s + Number(n || 0), 0) * p.price / 100
                );
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input
                      value={p.slug}
                      onChange={(e) => updateLocal(i, { slug: e.target.value })}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateLocal(i, { display_order: Number(e.target.value) })}
                      className="w-full px-3 py-2 rounded-lg bg-muted border-border text-sm"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input
                      type="number"
                      value={p.price}
                      onChange={(e) => updateLocal(i, { price: Number(e.target.value) })}
                      className="w-full px-3 py-2 rounded-lg bg-muted border-border text-sm"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      value={p.capacity ?? ""}
                      onChange={(e) =>
                        updateLocal(i, { capacity: e.target.value ? Number(e.target.value) : null })
                      }
                      className="w-full px-3 py-2 rounded-lg bg-muted border-border text-sm"
                    />
                  </div>
                ...
                <div className="flex flex-col gap-2">
                  <label className="flex items-center gap-2 text-sm">
                    <input
                      type="checkbox"
                      checked={p.active}
                      onChange={(e) => updateLocal(i, { active: e.target.checked })}
                    />
                  ...
                  <label className="flex items-center gap-2 text-sm">
                    <input
                      type="checkbox"
                      checked={p.partial_allowed}
                      onChange={(e) => updateLocal(i, { partial_allowed: e.target.checked })}
                    />
                  ...
                  {p.partial_allowed && (
                    <label className="flex items-center gap-2 text-sm pl-5">
                      <input
                        type="checkbox"
                        checked={p.allow_custom_amount}
                        onChange={(e) => updateLocal(i, { allow_custom_amount: e.target.checked })}
                      />
                      Allow user to type a custom amount
                      <span className="text-xs text-muted-foreground">
                        (uncheck to force fixed instalments only)
                      </span>
                    </label>
                  )}
                </div>
                ...
                {p.partial_allowed && (
                  <div className="space-y-3 p-3 rounded-lg bg-muted/40 border-border">
                    <div className="flex items-center gap-3">
                      ...
                      <select
                        value={p.installment_mode}
                        onChange={(e) => updateLocal(i, { installment_mode: e.target.value as any })}
                        className="px-2 py-1 rounded bg-background border-border text-xs"
                      >
                    ...
                    <label className="text-xs text-muted-foreground block font-semibold">
                      ...
                      {p.installment_mode === "percent"
                        ? "enter % of total price"
                        : "enter KES amount"}
                    </label>

                    {p.installments.length === 0 && (
                      <p className="text-xs text-muted-foreground italic">
                        No instalments added yet. Click "Add Instalment" below.
                      </p>
                    )}
                    ...
                    <p className={`text-xs font-semibold ${matches ? "text-emerald-400" : "text-yellow-400"}`}>
                      Total: KES {installmentTotal.toLocaleString()}{" "}
                      {matches
                        ? "✓ matches package price"
                        : `≠ package price KES ${Number(p.price).toLocaleString()} — adjust to match`}
                    </p>
                  </div>
                )}
