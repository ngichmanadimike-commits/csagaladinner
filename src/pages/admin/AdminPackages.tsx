            ...
            <div key={p.id || `new-${i}`} className="glass rounded-xl p-5 grid md:grid-cols-2 gap-4">
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input value={p.slug} onChange={(e) => updateLocal(i, { slug: e.target.value })} placeholder="individual" className="w-full px-3 py-2 rounded-lg bg-muted border border-border text-sm" />
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground">Order</label>
                    <input type="number" value={p.display_order} onChange={(e) => updateLocal(i, { display_order: Number(e.target.value) })} className="w-full px-3 py-2 rounded-lg bg-muted border border-border text-sm" />
                  </div>
                </div>
                <div>
                  <label className="text-xs text-muted-foreground">Name</label>
                  <input value={p.name} onChange={(e) => updateLocal(i, { name: e.target.value })} className="w-full px-3 py-2 rounded-lg bg-muted border border-border text-sm" />
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-xs text-muted-foreground">Price (KES)</label>
                    <input type="number" value={p.price} onChange={(e) => updateLocal(i, { price: Number(e.target.value) })} className="w-full px-3 py-2 rounded-lg bg-muted border border-border text-sm" />
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground">Capacity (optional)</label>
                    <input type="number" value={p.capacity ?? ""} onChange={(e) => updateLocal(i, { capacity: e.target.value ? Number(e.target.value) : null })} className="w-full px-3 py-2 rounded-lg bg-muted border border-border text-sm" />
                  </div>
                </div>
                <div>
                  <label className="text-xs text-muted-foreground">Description</label>
                  <input value={p.description || ""} onChange={(e) => updateLocal(i, { description: e.target.value })} className="w-full px-3 py-2 rounded-lg bg-muted border border-border text-sm" />
                </div>
                <div>
              ...
              <div className="space-y-3">
                ...
                {p.partial_allowed && (
                  <div className="space-y-2 p-3 rounded-lg bg-muted/40 border border-border">
                    <div className="flex items-center gap-3">
                      ...
                      <select value={p.installment_mode} onChange={(e) => updateLocal(i, { installment_mode: e.target.value as any })} className="px-2 py-1 rounded bg-background border border-border text-xs">
                        <option value="amount">Fixed amounts (KES)</option>
                        <option value="percent">Percent of price</option>
                      </select>
                    </div>
                    <label className="text-xs text-muted-foreground block">Installments (comma separated)</label>
                    <input
                      value={p.installments.join(",")}
                      onChange={(e) => updateLocal(i, { installments: e.target.value.split(",").map((s) => Number(s.trim()) || 0).filter((n, idx, arr) => arr[idx] !== undefined) })}
                      placeholder="1050,800,800"
                      className="w-full px-3 py-2 rounded-lg bg-background border border-border text-sm font-mono"
                    />
                    <p className={`text-xs ${matches ? "text-emerald-400" : "text-yellow-400"}`}>
                      Sum of installments = KES {installmentTotal.toLocaleString()} {matches ? "✓ matches package price" : `≠ KES ${Number(p.price).toLocaleString()}`}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      Tip: when using <em>Percent</em>, installments scale automatically with the price (e.g. 40,30,30 = 100% of price). When using <em>Fixed amounts</em>, the sum should equal the package price.
                    </p>
                  </div>
                )}
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <button onClick={() => remove(i)} className="px-3 py-2 rounded-lg border border-border text-destructive hover:bg-destructive/10 text-sm">
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
