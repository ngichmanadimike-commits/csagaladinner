...
const AdminRegistrations = () => {
  ...
  // FIX: Read from "registrations" table — ticket_purchases does not exist
  ...
  // FIX: Delete registration + all linked payments (cascade)
  ...
  const handleDeleteSelected = async () => {
    ...
    if (
      !confirm(
        `Delete ${selectedIds.length} registration(s) and ALL their payment records? This cannot be undone.`
      )
    )
      return;
  ...
  const handleDeleteRow = async (row: Registration) => {
    if (
      !confirm(
        `Delete booking ${row.ticket_code || row.id} for ${row.name}?\n\nThis will also delete ALL payment records linked to this booking. Cannot be undone.`
      )
    )
      return;
  ...
  // Revenue stats — only count confirmed/paid registrations
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search name, email, phone, booking code..."
              value={search}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        ...
        <div className="glass rounded-xl p-3">
          ...
          <p className="text-xl font-bold text-emerald-400">
            KES {confirmedRevenue.toLocaleString()}
          </p>
        </div>
        <div className="glass rounded-xl p-3">
        ...
        <div className="glass rounded-xl p-3">
          ...
          <p className="text-xl font-bold text-primary">
            {registrations.filter((r) => r.ticket_issued).length}
          </p>
        </div>
      </div>
