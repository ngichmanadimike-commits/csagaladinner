...
const AdminRegistrations = () => {
  ...
  const markAsPaid = async (id: string) => {
    ...
    else { toast.success("Marked as paid"); await load(); }
    setUpdating(null);
  };
          ...
          <div className="flex gap-2">
            {["all", "paid", "pending", "failed"].map((f) => (
              <button key={f} onClick={() => setFilter(f)}
                ...
                style={{ backgroundColor: filter === f ? "#D4AF37" : "rgba(255,255,255,0.05)", color: filter === f ? "#0A2342" : "rgba(255,255,255,0.5)" }}>
                {f}
              </button>
            ))}
          </div>
                ...
                {loading ? (
                  Array.from({ length: 5 }).map((_, i) => (
                    <tr key={i}>{Array.from({ length: 9 }).map((_, j) => (
                      <td key={j} className="px-4 py-3"><div className="h-4 rounded animate-pulse" style={{ backgroundColor: "rgba(255,255,255,0.05)" }} /></td>
                    ))}</tr>
                  ))
                ) : filtered.length === 0 ? (
                  <tr><td colSpan={9} className="px-4 py-10 text-center text-white/30">No registrations found</td></tr>
                ) : (
                  filtered.map((reg) => (
                    <tr key={reg.id} className="border-t transition-colors hover:bg-white/2" style={{ borderColor: "rgba(255,255,255,0.04)" }}>
                      <td className="px-4 py-3 text-white font-medium whitespace-nowrap">{reg.name || reg.full_name}</td>
                      <td className="px-4 py-3 text-white/60 text-xs"><p>{reg.email}</p><p>{reg.phone}</p></td>
                      <td className="px-4 py-3 text-white/70 text-xs max-w-[120px]">{reg.package_type || reg.ticket_type || "—"}</td>
                      <td className="px-4 py-3 text-white whitespace-nowrap">KES {Number(reg.total_cost || reg.amount || 0).toLocaleString()}</td>
                      <td className="px-4 py-3 text-white/60 text-xs font-mono">{reg.mpesa_receipt_number || "—"}</td>
                      <td className="px-4 py-3">
                        <span className="px-2 py-0.5 rounded-full text-xs font-semibold"
                          style={{ color: statusColor[reg.payment_status] || "#fff", backgroundColor: `${statusColor[reg.payment_status] || "#666"}20` }}>
                          {reg.payment_status}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        {reg.ticket_code ? (
                          <a href={`/ticket/${reg.ticket_code}`} target="_blank" rel="noopener noreferrer"
                      ...
                      <td className="px-4 py-3 text-white/40 text-xs whitespace-nowrap">{new Date(reg.created_at).toLocaleDateString()}</td>
                      <td className="px-4 py-3">
                        {reg.payment_status !== "paid" && (
                          <button onClick={() => markAsPaid(reg.id)} disabled={updating === reg.id}
